/**
 * GitHub Installation Token Edge Function
 * 
 * Generates a short-lived installation access token for GitHub API operations.
 * Used by the backend agent to access repos on behalf of the user.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    if (installError || !installation) {
      return jsonResponse({ error: 'No GitHub App installation found' }, 404)
    }
    ...
    if (!response.ok) {
      ...
      return jsonResponse({ error: 'Failed to generate access token' }, 500)
    }
